...
def min_play(board, token, ply_remaining):
    ...
    bestMoveForTHEM = min(moves, key=lambda x: x[1])
...
#     else:
#         (min_move, value) = min_play(newboard, token, ply_remaining)
#     moves.append((col, value))
# random.shuffle(moves)
# best_moves = max(moves, key=lambda x: x[1])
# return best_moves
# if newboard.is_full():
#    value = state_score(newboard, token)
# else:
#    (min_move, value) = min_play(newboard, other(token), ply_remaining-1)
# moves.append((col, value))
